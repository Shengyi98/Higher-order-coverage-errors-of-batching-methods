...
gamma[0, 0, 1] = math.sqrt(2)
gamma[1, 0, 0] = math.sqrt(2)
gamma[0, 1, 0] = math.sqrt(2)
...
for K in range(2, 31):
    ...
    for rep in range(n_rep):
        ...
        ER_3 = 1 / 2 * (
                    pdf0 * (-(F_pos - mu) / tsigma0) * (yx ** 2) - pdf0_neg * (-(F_neg - mu) / tsigma0) * (yx_neg ** 2))
    ...
    for rep in range(n_rep2):
        ...
        X1 = np.random.normal(size=(K, n))
        X2 = (np.square(X1) - 1) / math.sqrt(2)
        X = np.zeros((K, n, 2))
        X[:, :, 0] = X1
        X[:, :, 1] = X2
    ...
    print('Batching: ' + str(B_actuals[K]) + ' Sectioning ' + str(S_actuals[K]) + ' Sectioned Jackknife ' + str(
        SJ_actuals[K]))
    print('BatchingT: ' + str(level + 1 / n * (Edgeworth_errors[K, 0] + B_theos[K, 0])) + ' SectioningT ' + str(
        level + 1 / n * (Edgeworth_errors[K, 0] + S_theos[K, 0])) + ' Sectioned JackknifeT ' + str(
        level + 1 / n * (Edgeworth_errors[K, 0] + SJ_theos[K, 0])))

np.savez('results_normal', S=S_theos, B=B_theos, SJ=SJ_theos, E=Edgeworth_errors, SA=S_actuals, BA=B_actuals,
         SJA=SJ_actuals)
