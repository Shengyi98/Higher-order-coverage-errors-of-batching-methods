...
for K in range(2,31):
    ...
    def edge_poly_2(x):
        return 1/24*np.einsum('ijkl,im,jn,ko,lp,m,n,o,p',kappa,sigmainv,sigmainv,sigmainv,sigmainv,x,x,x,x) \
                -1/4*np.einsum('ijkl,im,jn,kl,m,n',kappa,sigmainv,sigmainv,sigmainv,x,x) \
                +1/8*np.einsum('ijkl,ij,kl',kappa,sigmainv,sigmainv) \
                +1/72*np.einsum('ijk,lmn,io,jp,kq,lr,ms,nt,o,p,q,r,s,t',gamma,gamma,sigmainv,sigmainv,sigmainv,sigmainv,sigmainv,sigmainv,x,x,x,x,x,x) \
                -6/72*np.einsum('ijk,lmn,io,jp,kq,lr,mn,o,p,q,r',gamma,gamma,sigmainv,sigmainv,sigmainv,sigmainv,sigmainv,x,x,x,x) \
                -9/72*np.einsum('ijk,lmn,io,jp,lq,mr,kn,o,p,q,r',gamma,gamma,sigmainv,sigmainv,sigmainv,sigmainv,sigmainv,x,x,x,x) \
                +9/72*np.einsum('ijk,lmn,io,lp,jk,mn,o,p',gamma,gamma,sigmainv,sigmainv,sigmainv,sigmainv,x,x) \
                +18/72*np.einsum('ijk,lmn,io,jp,kl,mn,o,p',gamma,gamma,sigmainv,sigmainv,sigmainv,sigmainv,x,x) \
                +18/72*np.einsum('ijk,lmn,io,lp,jm,kn,o,p',gamma,gamma,sigmainv,sigmainv,sigmainv,sigmainv,x,x) \
                -9/72*np.einsum('ijk,lmn,ij,kl,mn',gamma,gamma,sigmainv,sigmainv,sigmainv) \
                -6/72*np.einsum('ijk,lmn,il,jm,kn',gamma,gamma,sigmainv,sigmainv,sigmainv)
    ...
    n_rep0 = 1000000 #suggest 1,000,000
    E_estimate0 = 0
    ER_0s = np.zeros((n_rep0,))
    start = time.time()
    for rep in range(n_rep0):
        # generate data
        X_aves_scaled = np.random.multivariate_normal(size=(K,),mean = np.zeros((dim,)),cov=sigma);
        A = np.average(X_aves_scaled,axis=0);
        B = np.zeros((K,dim));
        ...
        for i in range(K):
            ...
            E2 = E2 + np.inner(u,B[i,:])**2
        qE2 = math.sqrt(E2)
        obj_val = math.sqrt(K*(K-1))*np.inner(u,A)/qE2
        ...
        if (-q<obj_val)&(obj_val<q):
            ER_0 = 0
            for i in range(K):
                p1s[i] = edge_poly_1(X_aves_scaled[i,:])
            for i in range(K):
                for j in range(i+1,K):
                    ER_0 = ER_0 + p1s[i]*p1s[j]
            for i in range(K):
                ER_0 = ER_0 + edge_2(X_aves_scaled[i,:])

    #         print(ER_0)
            ER_0s[rep] = ER_0
            E_estimate0 = E_estimate0 + ER_0
    end = time.time()
    print(end-start)
    Edgeworth_errors[K,0] = np.average(ER_0s)
    Edgeworth_errors[K,1] = 1.96/math.sqrt(n_rep0)*np.std(ER_0s)
    ...
    #     X = np.random.multivariate_normal(size=(K,n),mean = np.zeros((dim,)),cov=sigma);
        X1 = np.random.exponential(size=(K,n)) - 1
        X2 = (chi2.rvs(df=1,size=(K,n)) - 1)/math.sqrt(2)
        X = np.zeros((K,n,2))
        X[:,:,0] = X1
        X[:,:,1] = X2
    ...
    print('Batching: '+ str(B_actuals[K])+ ' Sectioning '+str(S_actuals[K])+' Sectioned Jackknife ' +str(SJ_actuals[K]))
    print('BatchingT: '+ str(level+1/n*(Edgeworth_errors[K,0]+B_theos[K,0]))+ ' SectioningT '+str(level+1/n*(Edgeworth_errors[K,0]+S_theos[K,0]))+' Sectioned JackknifeT ' +str(level+1/n*(Edgeworth_errors[K,0]+SJ_theos[K,0])))

np.savez('results',S=S_theos,B=B_theos,SJ=SJ_theos,E=Edgeworth_errors,SA =S_actuals, BA = B_actuals, SJA = SJ_actuals)